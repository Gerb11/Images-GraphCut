...
const char*  s_values[]   = {"s=40.0" ,  "s=2.0"  , "s=5.0" ,  "s=6.0"  , "s=6.0", "s=8.0" , "s=4.0"}; // default values for "s" parameter for different images
const char* image_names[] = { "logo" , "uwocampus", "canada", "Im_R" , "ImGr_R" , "Michel_R" , "squareCircle_R"}; // an array of image file names
int im_index = 5;    // index of currently opened image (inital value)
int save_counter[] = {0,0,0,0,0,0}; // counter of saved results for each image above 
const char* mode_names[]  = { "SEGMENT mode", "STEREO mode", "YOUR mode"}; // an array of mode names 
...
Mode mode=SEGMENT; // stores current mode (index in the array 'mode_names')
...
int main()
{
	...
	int dropList_files = CreateDropList(7, image_names, im_index, image_load); // the last argument specifies the call-back function, see "cs1037utils.h"
...
void mode_set(int index)
{
	...
	graph.reset(image,s, im_index,mode);
	draw();
	if(mode == STEREO) {
		cut();
	}
...
void image_load(int index) 
{
    ...
    SetControlPosition(check_box_view,image.getWidth()+pad+5, cp_height+pad+60);
...
void reset() { 
	graph.reset(image,s, im_index, mode,false); // clears current segmentation but keeps seeds - function in graphcuts.cpp
	draw();
	if(mode == STEREO) {
		cut();
	}
...
{ 
	...
	if ((mode==STEREO) && !graph.getLabeling().isEmpty()) drawImage(graph.getLabeling(),colors,transparancy1); // 4th draw() function in Image2D.h
